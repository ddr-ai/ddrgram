...
import "./_runtime.mjs";
import { nt as init_dist, tt as Buffer } from "./_libs/@tanstack/react-router+[...].mjs";
import { d as init_dist$1, f as process$1 } from "./_libs/@floating-ui/react-dom+[...].mjs";
import { i as toEventHandler, n as HTTPError, o as NodeResponse, r as defineLazyEventHandler, t as H3Core } from "./_libs/h3+[...].mjs";
import { n as require_path_browserify } from "./_libs/node-localstorage+[...].mjs";
...
var grokOgIdentity = { "site": {
	"title": "Telegram Video Browser",
	"type": "website",
	"card": "custom",
	"description": "A quiet dark shelf for Telegram channel videos.",
	"image": "/og.jpg"
} };
//#endregion
...
init_dist();
init_dist$1();
var import_path_browserify = require_path_browserify();
...
function resolvePublicHost(hostHeader) {
	return publicAppHost(process$1.env?.VITE_PUBLIC_HOSTNAME) || publicAppHost(hostHeader);
}
...
function readGrokProjectId() {
	const fromProcess = typeof process$1 !== "undefined" ? process$1.env?.VITE_PROJECT_ID : "";
	return String(fromProcess ?? "").trim();
}
function readXCreator() {
	const fromProcess = typeof process$1 !== "undefined" ? process$1.env?.X_CREATOR : "";
	return String(fromProcess ?? "").trim();
}
function readXCreatorId() {
	const fromProcess = typeof process$1 !== "undefined" ? process$1.env?.X_CREATOR_ID : "";
	return String(fromProcess ?? "").trim();
}
...
function readOgSite(cwd = process$1.cwd()) {
	try {
		const raw = readFileSync((0, import_path_browserify.join)(cwd, OG_SITE_REL_PATH), "utf8");
...
function ogCardPublicPath(cwd = process$1.cwd()) {
	if (existsSync((0, import_path_browserify.join)(cwd, "public/og.jpg"))) return "/og.jpg";
	if (existsSync((0, import_path_browserify.join)(cwd, "public/og.png"))) return "/og.png";
	return "";
}
function detectCustomOgCard(cwd = process$1.cwd(), site = {}) {
...
function snapshotOgIdentity(cwd = process$1.cwd()) {
	...
	if (existsSync((0, import_path_browserify.join)(cwd, "public/x-banner.jpg"))) site.banner = site.banner || "/x-banner.jpg";
	return { site };
}
function ogServiceUrl() {
	return (String(process$1.env?.VITE_OG_SERVICE_URL ?? "").trim() || "https://og.grok.me").replace(/\/+$/, "");
}
...
*/
function resolveOgCardAsset(site = {}, cwd = process$1.cwd()) {
...
function grokOgHeadTags({ host = "", appName = DEFAULT_APP_NAME, site = {}, documentTitle = "", cwd = process$1.cwd() } = {}) {
...
function normalizeHeadContext(ctx = {}) {
	const cwd = ctx.cwd ?? process$1.cwd();
